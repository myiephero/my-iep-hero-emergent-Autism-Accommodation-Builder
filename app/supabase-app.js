...
import { AuthProvider, AuthGuard, useAuth, UserProfile } from '@/components/AuthComponents'
...
const MainApp = () => {
  ...
  const loadUserSessions = async () => {
    try {
      const response = await fetch(`/api/sessions/${user.id}`, {
  ...
  const loadAssignedParents = async () => {
    try {
      // Get parents assigned to this advocate from Supabase
      const response = await fetch(`/api/advocate/assigned-parents`, {
        headers: getAuthHeaders()
      })
      if (response.ok) {
        const parents = await response.json()
        setAssignedParents(parents)
      }
    } catch (error) {
      console.error('Failed to load assigned parents:', error)
    }
  }
  ...
  const toggleApproval = async (approved) => {
    if (profile.role !== 'advocate') return
        ...
        <html>
          ...
          <body>
            ...
            <div class="header-info">
              ...
              <p><strong>Plan Type:</strong> ${profile?.plan_type?.toUpperCase() || 'FREE'}</p>
            </div>
  ...
  const assignToStudent = () => {
    const assignmentText = `STUDENT ASSIGNMENT - ${formData.childName}\n` +
      ...
      `Plan Type: ${profile?.plan_type?.toUpperCase() || 'FREE'}\n` +
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-green-50 to-orange-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="container mx-auto px-4 py-4">
          ...
          {profile.role === 'advocate' && assignedParents.length > 0 && (
              ...
              <div className="flex items-center justify-between">
                ...
                <Select 
                  ...
                  onValueChange={(parentId) => {
                    const parent = assignedParents.find(p => p.id === parentId)
                    ...
                    {assignedParents.map(parent => (
                      <SelectItem key={parent.id} value={parent.id}>
                        <div className="flex items-center space-x-2">
                          ...
                          <Badge variant="outline">{parent.plan_type}</Badge>
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        {/* Hero Plan Features - Only show for Hero users */}
        {profile.plan_type === 'hero' && (
          <div className="mb-8 grid gap-6">
            <div className="grid md:grid-cols-2 gap-6">
              <AdvocateRecommendations currentUser={profile} />
              <DocumentVault currentUser={profile} />
            </div>
          </div>
        )}
        ...
        {/* Main Content - Steps and Results */}
        <Card className="shadow-lg border-0 mb-8">
          <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-lg">
            <CardTitle className="text-2xl flex items-center space-x-2">
              <span>{steps[currentStep].title}</span>
              {profile.role === 'advocate' && selectedParent && (
                <Badge className="bg-white/20 text-white">
                  for {selectedParent.first_name} {selectedParent.last_name}
                </Badge>
              )}
            </CardTitle>
              ...
              <div className="space-y-6">
                {isGenerating ? (
                  ...
                  <>
                    ...
                    {profile.role === 'advocate' && currentSession && (
                    ...
                    {/* Hero Plan Features - Advanced Review & Team Collaboration */}
                    {profile.plan_type === 'hero' && currentSession && (
                      <div className="space-y-6">
                        <AdvancedAIReview 
                          session={currentSession} 
                          currentUser={profile}
                          onReviewComplete={(review) => {
                            toast.success('Advanced review completed!')
                          }}
                        />
                        <TeamCollaboration 
                          currentUser={profile} 
                    ...
                    {(selectedParent?.plan_type === 'free' || (!selectedParent && profile.plan_type === 'free')) && (
                      <Alert className="border-orange-200 bg-orange-50">
                        <Star className="h-4 w-4 text-orange-600" />
                        <AlertDescription className="text-orange-800">
                          <div className="flex items-center justify-between">
                            <div>
                              <strong>Unlock 2x more accommodations!</strong> Upgrade to HERO Plan for advanced AI review, legal risk scan, priority advocate pairing, and team collaboration.
                            </div>
                            <Button 
          ...
          {currentStep < 3 && (
            <div className="flex justify-between items-center p-6 bg-gray-50 rounded-b-lg">
              ...
              <Button
                onClick={nextStep}
                disabled={!canProceed() || isGenerating || (profile.role === 'advocate' && !selectedParent)}
